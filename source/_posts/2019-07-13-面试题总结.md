...
copyright: true
permalink: 1
date: 2019-07-13 19:48:20
categories: 面试
description:
---

​	![](https://i.loli.net/2019/07/13/5d29d3143b9f875943.png)

​	对于一些前端面试题的收集整理，个人能力有限，难免有疏漏之处，烦请批评指正。
...
```
  ...
  *   可继承的样式： font-size font-family color line-height text-indent text-align;
...
```
line-height可以通过继承来的fontsize等属性获得与之相匹配的line-height，这也是许多inline-block元素使用过程中出现各种bug的罪魁祸首，所有使用时一定要小心。
```
...
```
1.一般样式都是放在 head 之间的。
2.写在head标签中利于浏览器逐步渲染（resources downloading->CSSOM+DOM->RenderTree(composite)->Layout->paint）
3.写在body标签后由于浏览器以逐行方式对html文档进行解析，当解析到写在尾部的样式表（外联或写在style标签）会导致浏览器停止之前的渲染，等待加载且解析样式表完成之后重新渲染，在windows的IE下可能会出现FOUC现象（即样式失效导致的页面闪烁问题）
```

### script写在body后与body前有什么区别

```
HTML 2.0起放在“body标签闭合之后”就是不合标准的。但是浏览器却不会报错，是因为如果在“body标签闭合之后”后再出现script或任何元素的开始标签，都是parse error，浏览器会忽略之前的，即视作仍旧在body内。所以实际效果和写在“body标签闭合之前”之前是没有区别的。所以要么放head标签里，要么放 </body> 之前
```
...
```
	rem（font size of the root element）是指相对于根元素(html)的字体大小的单位。简单的说它就是一个相对单位。看到rem大家一定会想起em单位，em（font size of the element）是指相对于父元素的字体大小的单位。
```
